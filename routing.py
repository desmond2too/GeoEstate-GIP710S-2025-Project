...
# OSRM API endpoint - using a free public instance
...
def get_route(start_lat, start_lng, end_lat, end_lng):
    ...
    try:
        ...
        # Build the URL.  OSRM uses lng,lat order and not lat,lng, it wont run otherwise
        ...
        # Convert to [latitude, longitude] for consistency with the  web app
...
def fallback_route_calculation(start_lat, start_lng, end_lat, end_lng):
    ...
    Returns:
        dict: Dictionary with route information
    """
    # Fallback to a simple straight-line calculation (for people on foot) meant for shorter distances or ares with no roads 
